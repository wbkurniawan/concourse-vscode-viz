...
function filterAndUpdatePipeline() {
    ...
    if (activeGroup !== null) {
        filteredJobs = pipelineJobs.filter(job => job.groups && Array.isArray(job.groups) && job.groups.includes(activeGroup));
...
function update(svg, rawYaml) {
    ...
    try {
        ...
        if (activeGroup !== null) {
            ...
            filteredJobs = jobs.filter((job) => job.groups && Array.isArray(job.groups) && job.groups.includes(activeGroup));
            console.log(`Filtered to ${filteredJobs.length} jobs in group: ${activeGroup}`);
        }
